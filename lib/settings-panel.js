...
// The colophon at the foot of Settings — the same markup in the popup and the
// side panel, so it's wired here with the rest of the shared settings UI.
//
// These are real <a href> elements for the sake of middle-click, "copy link"
// and screen readers, but following one in place would be wrong in both hosts:
// in the side panel it would navigate the workbook away from itself, and in the
// popup it would load a page into a window that closes the moment focus moves.
// So the click is intercepted and the page opens in a tab instead. An href of
// its own is still what's opened — no second copy of these paths to keep in
// sync.
function wireAboutLinks() {
  document.querySelectorAll(".about__links a").forEach((link) => {
    link.addEventListener("click", (e) => {
      // Leave modified clicks alone — that's the user asking for a new tab or
      // window themselves, and Chrome handles those correctly already.
